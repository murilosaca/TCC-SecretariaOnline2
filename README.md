...
**Estado atual: P0 fechado e demonstrável** em `main`. O circuito oficial do MVP: login → primeiro acesso/LGPD → dashboard do aluno → nova solicitação → presença SECRET_SINGLE (aluno + hospedeiro professor) → formativa `PENDENTE_CONFIRMACAO` → consulta pública de protocolo.

**Próxima fatia: deliberação do motor de solicitações** (item 2 do plano). Não abrir QR, CAAF em lote, COE nem certificado neste sprint.
...
| BFF | `modules/bff/` | `GET /bff/dashboard/aluno` — agrega identidade, período, solicitações, eventos e formativas. Degrada por bloco (HTTP 200). 403 se a sessão não for de aluno |
| Formativas | `modules/formativas/` | Tabela `formativa` (V007). Gatilho **na mesma TX** de `ConfirmarPresencaUseCase` (SECRET_SINGLE + ENTRADA): após persistir presença, `FormativaPorPresencaPort` cria `PENDENTE_CONFIRMACAO` via `FormativaRepository` (UNIQUE `id_evento+id_aluno`, idempotente). Aluno confirma → `AGUARDANDO_CAAF`. Sem parecer CAAF, sem comprovante, sem certificado. Outbox `formativa.criada` / `formativa.confirmada` no mesmo TX |
...
| `/bff/dashboard/aluno` | JWT + `dashboard.view_own` + capability de aluno | Dashboard agregado (`horasFormativas` via query; validadas ainda 0 até CAAF) |
| `/formativas` | JWT + `formative.view_own` / `formative.confirm_own` | Lista/detalhe; confirmar e cancelar. Sem POST avulso |
...
| 5 | Certificados oficiais + F0.7 | Depois de 3 (precisa de `APROVADA`) |
...
Tabela `formativa` (V007). Gatilho na mesma TX de `ConfirmarPresencaUseCase` (`SECRET_SINGLE` + `ENTRADA`). Aluno confirma (`PENDENTE_CONFIRMACAO` → `AGUARDANDO_CAAF`) ou cancela. Sem `POST /formativas`, sem comprovante, sem parecer CAAF, sem certificado. BFF: `kpis.horasFormativas` soma `APROVADA` (ainda 0) e até 3 `pendenciasFormativas`.
...
Proxies Vite (`/auth`, `/academico`, `/publico`, `/requests`, `/request-types`, `/bff`, `/events`, `/formativas`, `/v3`, `/swagger-ui`, `/actuator`) vão para `http://localhost:8080`. CORS libera `http://localhost:5173` e `http://localhost:5174`.
...
7. **Formativas** — professor cria SECRET_SINGLE, abre janela (`GET /formativas` → 403). Aluno confirma PIN → formativa `PENDENTE_CONFIRMACAO` em `/formativas` e CTA no `/inicio` → `_links.confirmar` → `AGUARDANDO_CAAF` (botões somem; CTA some no refetch). Horas `0 / 120`. `novo.dev` em `/formativas` volta ao gate. Login é e-mail (`aluno.dev@ufpr.br`) ou GRR — `professor.dev` sozinho não é identificador válido. F5 na rota `/formativas` deve recarregar a SPA (proxy com bypass `Accept: text/html`), não a API.

Horas formativas no dashboard passam pela query do módulo `formativas` (`validadas` ainda 0 até o item 3 — CAAF individual). Certificados continuam `null`. F0.7 permanece stub.
