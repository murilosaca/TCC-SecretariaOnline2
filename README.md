...
- Cursos: lista/GET/PUT/DELETE só do conjunto; POST cria, inclui o criador em `curso_secretario` na mesma TX (mesmo se `secretariosIds` vier vazio) e grava o restante. Cross-curso → **404** (não vaza existência). Sem `user.manage_all`. `_links.atualizar`/`excluir` do item só aparecem se o curso estiver no escopo real — o `true` literal do POST foi removido. `_links.disciplinas` navega para `GET /academico/disciplinas?idCurso={id}` (não há sub-recurso `/academico/cursos/{id}/disciplinas`).
