...
| `pages/secretaria/` | CRUD acadêmico F5.6–F5.9 (nav só se `/auth/me`._links; form “Novo …” só com `_links.criar`) |
...
- `request.view_curso`: fila e GET/transição cujo aluno (join IAM por GRR ou e-mail institucional) não está nos `cursoIds` → fora da lista / 404. Professor só com `request.deliberate` **não** ganha esse filtro. Secretaria seed tem `request.triage` + `request.deliberate` (reusa `/solicitacoes?to=me`); o dispatcher **não** manda deep-link porque tem triage. O rel de menu `solicitacoes` exige `request.view_own` — a secretaria **não** recebe esse item (a tela renderizada seria a do aluno e o `GET /requests` sem `canDeliberate` responde 403). `request.view_curso` e `request.deliberate` geram `deliberar`. **F5.2 (fila central da secretaria) ficou de fora**: é P2, tela própria, e esta fatia é só correção — não implementar a fila.
...
- UI: `AppLayout` via `useActions(links)`. Sem `authorities.includes`. Form “Novo …” some sem `_links.criar`.
...
O seed IAM (`@Profile("dev")`, `iam.seed.enabled`) **substitui** o conjunto de authorities (revoga extras que não estão na lista canônica) e registra no log. O seed acadêmico respeita o mesmo `iam.seed.enabled` e **não** apaga secretários extras de TADS (só adiciona `secretaria.dev` se ausente). No POST de curso, o criador entra em `curso_secretario` mesmo com `secretariosIds` vazio (F5.7-D01). No PUT, `replaceAll` só corre se `secretariosIds` vier no body; o chamador só é reinserido se **já** for secretário — coordenador que edita (escopo só por `id_coordenador`) não ganha linha em `curso_secretario`.
