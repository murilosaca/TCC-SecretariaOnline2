...
- Cursos: lista/GET/PUT/DELETE só do conjunto; POST cria, inclui o criador em `curso_secretario` na mesma TX (mesmo se `secretariosIds` vier vazio) e grava o restante. Cross-curso → **404** (não vaza existência). Sem `user.manage_all`. `_links.atualizar`/`excluir` do item só aparecem se o curso estiver no escopo real — o `true` literal do POST foi removido.
- Disciplinas e alunos: listagens filtradas a `id_curso IN cursoIds`; writes fora do escopo → 404. O `podeGerenciar` da listagem é tautológico (o use case já restringiu); `_links.criar` da coleção é incondicional porque o endpoint já exige a capability `manage`. Nesta fatia as quatro capabilities (`course.manage`, `subject.manage`, `user.manage_students`, `calendar.manage`) andam juntas no seed — Alunos/Disciplinas reusam `GET /academico/cursos` (exige `course.manage`); sem ela o combo fica vazio e o form é desabilitado. Sem endpoint novo de busca (isso é F7.1/F7.2).
