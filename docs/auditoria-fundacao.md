...
| Flyway nas ITs | migrations imutáveis (última **V012** = estágio; V011 = `curso_secretario`) | `application-test.yml` usa H2 `ddl-auto: create-drop` e `flyway.enabled: false`. Toda IT é `@ActiveProfiles("test")`, então a V012 nunca é exercitada pelo Flyway; divergência DDL × entidade passa verde e só explode no boot contra o Postgres `:5433`. Em dev/prod o `ddl-auto` segue `validate` | Testcontainers + Postgres num perfil `it` numa fatia futura. **Não** implementar Testcontainers nesta correção |
...
| F4.2 pool COE | RF-F4-002 | Item 9.1 entregou parecer individual (RF-F3-005) em `/estagios?to=me`. Sem assign, sem BulkActionBar, sem `/commissions/coe` | Pool só atribui; nunca aprovar em lote |
| TCC | RF-F1-008 / RF-F3-006 | Sem módulo `tcc` | 9.2 |
...
| MinIO / `arquivos` | upload presigned | PDF de estágio e de certificado em `bytea`. Sem `modules/arquivos` e sem MinIO no compose | Quando o compose tiver MinIO |
