...
| Flyway nas ITs | migrations imutáveis (última **V014** = F6.1; V013 = TCC) | `application-test.yml` usa H2 `ddl-auto: create-drop` e `flyway.enabled: false`. Toda IT é `@ActiveProfiles("test")`, então a V014 nunca é exercitada pelo Flyway; divergência DDL × entidade passa verde e só explode no boot contra o Postgres `:5433`. Em dev/prod o `ddl-auto` segue `validate` | Testcontainers + Postgres num perfil `it` numa fatia futura. **Não** implementar Testcontainers nesta correção |
